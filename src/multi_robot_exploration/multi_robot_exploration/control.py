...
# RACER-inspired exploration module
from multi_robot_exploration.racer_exploration import (
    racer_explore,
...
class HeadquartersControl(Node):
    def __init__(self, num_robots, exploration_mode="racer"):
        """
        Args:
            num_robots: Number of robots in the fleet
            exploration_mode: "racer" (RACER-style) or "frontier" (original)
        """
        ...
        # RACER configuration
        if self.exploration_mode == "racer":
            self.racer_config = ExplorerConfig()
            self.get_logger().info("🚀 Using RACER-style exploration mode")
            # Coordination: publish planned targets for conflict resolution
            self.coord_pub = self.create_publisher(String, "/exploration/coordination", 10)
            self.coord_sub = self.create_subscription(
                String, "/exploration/coordination", self.coordination_callback, 10
            )
            self.planned_targets = {}  # {robot_name: (x, y)} — planned targets of all robots
        else:
            self.racer_config = None
            self.get_logger().info("📡 Using frontier-based exploration mode")
        ...
        self.odom_received = set()  # Track which robots have received odometry
        ...
        # Initialize planned targets for RACER mode
        if self.exploration_mode == "racer":
            for i in range(self.num_robots):
                self.planned_targets[f"tb{i+1}"] = None
    ...
    def robot_odom_callback(self, msg, robot_name):
        ...
        self.odom_received.add(robot_name)
    ...
    def start_exploration(self, robot_number):
        ...
        Supports two modes:
          - "frontier": Original simple frontier-based exploration
          - "racer": RACER-inspired pipeline (viewpoint sampling + TSP tour + coordination)
        """
        robot_name = f"tb{robot_number}"
        last_target = None  # To prevent sending the same goal repeatedly
        odom_logged = False  # One-time log flag
        while rclpy.ok():
            if self.map_data is not None:
                # Wait until odometry is received for this robot
                if robot_name not in self.odom_received:
                    time.sleep(2)
                    continue
                if not odom_logged:
                    self.get_logger().info(f"{robot_name} odometry ready at {self.robot_positions[robot_name]}")
                    odom_logged = True
                ...
                if self.robot_states[robot_name] == "idle":
                    if self.exploration_mode == "racer":
                        target = self._racer_explore(robot_name, last_target)
                    else:
                        target = exploration(
                            self.map_data,
                            self.map_width,
                    ...
                    if target:
                        ...
                        self.map_saved = False  # Reset flag: new exploration started
                        
                        # Broadcast planned target for coordination (RACER mode)
                        if self.exploration_mode == "racer":
                            self._broadcast_target(robot_name, target)
                        
                        self.send_goal(robot_name, target)
                    else:
                        self.get_logger().warn(f"{robot_name} found no valid target (pos: {robot_position})")
                else:
    ...
    def _racer_explore(self, robot_name, last_target=None):
        """RACER-style exploration: viewpoint sampling + TSP tour + coordination."""
        # Build robot_positions dict for all robots
        all_positions = dict(self.robot_positions)

        # Resolve conflicts: check if another robot already claimed this area
        target = racer_explore(
            self.map_data,
            self.map_width,
            self.map_height,
            self.resolution,
            ...
            robot_name,
            last_target=last_target,
            config=self.racer_config,
        )

        if target is None:
            return None

        # Conflict resolution with other robots' planned targets
        robot_targets = dict(self.planned_targets)
        robot_targets[robot_name] = target
        resolved = resolve_target_conflicts(robot_targets, self.racer_config.robot_conflict_radius)

        return resolved.get(robot_name)

    def _broadcast_target(self, robot_name, target):
        """Publish planned target to coordination topic for other robots."""
        msg = String()
        data = {
            'robot': robot_name,
        ...
        self.planned_targets[robot_name] = target

    def coordination_callback(self, msg):
        """Receive coordination messages from other robots."""
        try:
            data = json.loads(msg.data)
            robot = data['robot']
            target = tuple(data['target'])
            self.planned_targets[robot] = target
    ...
    def send_goal(self, robot_name, target):
        ...
        if robot_name in self.robot_nav_clients and self.robot_nav_clients[robot_name].wait_for_server(timeout_sec=2.0):
            ...
            send_goal_future = self.robot_nav_clients[robot_name].send_goal_async(goal, feedback_callback=self.feedback_callback)
            send_goal_future.add_done_callback(lambda f, r=robot_name: self.goal_response_callback(r, f))
        else:
            self.get_logger().warn(f"{robot_name} action server not available, retrying later")
            self.robot_states[robot_name] = "idle"

    def goal_response_callback(self, robot_name, future):
        """Callback when the goal has been accepted/rejected by the action server."""
        goal_handle = future.result()
        if not goal_handle.accepted:
            self.get_logger().warn(f"{robot_name} goal was rejected by server")
            self.robot_states[robot_name] = "idle"
            self.check_exploration_completion()
            return

        self.get_logger().info(f"{robot_name} goal accepted, navigating...")
        # Step 2: Wait for the navigation result
        result_future = goal_handle.get_result_async()
        result_future.add_done_callback(lambda f, r=robot_name: self.goal_result_callback(r, f))

    def feedback_callback(self, feedback_msg):
        """Handle feedback from the action server."""
        pass

    def goal_result_callback(self, robot_name, future):
        """Handle the final result of the navigation goal."""
        result = future.result()
        if result.status == GoalStatus.STATUS_SUCCEEDED:
            self.get_logger().info(f"{robot_name} reached its goal!")
        else:
            self.get_logger().warn(f"{robot_name} goal failed with status: {result.status}")
...
def main(args=None):
    ...
    node.declare_parameter('exploration_mode', 'racer')  # "racer" or "frontier"

    # Get the parameters
    robot_count = node.get_parameter('robot_count').get_parameter_value().integer_value
    exploration_mode = node.get_parameter('exploration_mode').get_parameter_value().string_value
    print(f"Number of robots: {robot_count}")
    print(f"Exploration mode: {exploration_mode}")
    control = HeadquartersControl(robot_count, exploration_mode=exploration_mode)  
